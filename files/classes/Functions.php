...
class Functions
{
    ...
    if ($json['inputs']['username']['validate'] === 'valid' && $json['inputs']['password']['validate'] === 'valid' && $json['inputs']['password_confirm']['validate'] === 'valid' && $json['inputs']['email']['validate'] === 'valid') {
      if ($mysqli->query('SELECT userId FROM player_accounts WHERE username = "' . $username . '"')->num_rows <= 0) {
        ...
        try {
          ...
          $json['message'] = 'You successfully registered, you can now login';
  ...
  public static function Buy($itemId, $amount)
  {
    ...
    $pet = $mysqli->query('SELECT * FROM player_pet WHERE userId = ' . $player['userId'] . '')->fetch_assoc();
    $modules = json_decode($pet["modules"]);
    if (isset($shop['items'][$itemId]) && $shop['items'][$itemId]['active']) {
      ...
      $data = json_decode($player['data']);
      $booty = json_decode($player['bootyKeys']);
      ...
      if (($shop['items'][$itemId]['priceType'] == 'uridium' ? $data->uridium : $data->credits) >= $price) {
        ...
        if ($shop['items'][$itemId]['name'] == 'Apis') {
          ...
          $json['message'] = 'You have purchased ' . $amount . ' ' . $shop['items'][$itemId]['name'];
          $status = true;
        ...
        } else if ($shop['items'][$itemId]['name'] == 'LF-4') {
          $items->lf4Count += $amount;
          $status = true; 

        } else if ($shop['items'][$itemId]['name'] == 'G3N-7900') {
        ...
        } else if ($shop['items'][$itemId]['name'] == 'E.C') {
          $data->jackpot += $amount;
          $status = true;
          
        } else if ($shop['items'][$itemId]['name'] == 'BootyKey') {
          $booty->greenKeys += $amount;
          $status = true;

        } else if ($shop['items'][$itemId]['name'] == 'DMG_B01') {
          $boosterTypeId = "2";
          $boosterType = "0";
        } else if ($shop['items'][$itemId]['name'] == 'DMG_B02') {
          $boosterTypeId = "2";
          $boosterType = "1";

        } else if ($shop['items'][$itemId]['name'] == 'HON_B01') {
          $boosterTypeId = "1";
          $boosterType = "5";

        } else if ($shop['items'][$itemId]['name'] == 'HON_B02') {
          $boosterTypeId = "1";
          $boosterType = "6";
        } else if ($shop['items'][$itemId]['name'] == 'XP_B01') {
          $boosterTypeId = "0";
          $boosterType = "2";
        } else if ($shop['items'][$itemId]['name'] == 'XP_B02') {
          $boosterTypeId = "0";
          $boosterType = "3";
        } else if ($shop['items'][$itemId]['name'] == 'HP_B01') {
          $boosterTypeId = "7";
          $boosterType = "8";
        } else if ($shop['items'][$itemId]['name'] == 'HP_B02') {
          $boosterTypeId = "7";
          $boosterType = "9";
        } else if ($shop['items'][$itemId]['name'] == 'SHD_B01') {
          $boosterTypeId = "3";
          $boosterType = "15";
        } else if ($shop['items'][$itemId]['name'] == 'SHD_B02') {
          $boosterTypeId = "3";
          $boosterType = "16";
        } else if ($shop['items'][$itemId]['name'] == 'CD_B01') {
          $boosterTypeId = "8";
          $boosterType = "21";
        } else if ($shop['items'][$itemId]['name'] == 'CD_B02') {
          $boosterTypeId = "8";
          $boosterType = "22";
        } else if ($pet != null && $shop['items'][$itemId]['name'] == 'REX_LEVEL_' . $pet['level'] + 1) {
          $json["message"] = "ici";
          $pet['level'] = $pet['level'] + 1;
          $status = true;
        } else if($pet == null && $shop['items'][$itemId]['name'].include("REX_LEVEL_") == true)
        {
          $status = true;
        }
        else if ($modules != null && $shop['items'][$itemId]['name'].include("CDR"))
        {
          ...
          $pet["modules"] = json_encode($modules);
          $status = true;
        }
        else if ($modules == null && $shop['items'][$itemId]['name'].include("CDR"))
        {
        ...
        if ($boosterType != -1 && $boosterTypeId != -1) {
          $newBooster = new stdClass();
          foreach ($boosters as $key => $value) {
            $newBooster->{$key} = $value;
          }
          if (!isset($boosters->{$boosterTypeId})) {

            $newBooster->{$boosterTypeId} = [["Type" => $boosterType, "Seconds" => "3600"]];
            $json["message"] = json_encode($newBooster);
          } else {
            $found = false;
            //add time to booster type 2 
            foreach ($newBooster->{$boosterTypeId} as $key => $value) {
            ...
            if (!$found) {
              array_push($newBooster->{$boosterTypeId}, ["Type" => $boosterType, "Seconds" => "3600"]);
            }
          }
          $status = true;
        }
        ...
        if ($status) {
          ...
          try {
            ...
            $mysqli->query("UPDATE player_equipment SET boosters = '" . json_encode($newBooster) . "' WHERE userId = " . $player['userId'] . "");

            if (
              $pet != null
            )
            {
              $mysqli->query("UPDATE player_pet SET `level` =" . $pet['level'] . ",`lasersCount` =" . $pet['lasersCount'] . ", `shieldCount` = " . $pet['shieldCount'] . ", `modules` = '".$pet['modules']."'  WHERE userId = " . $player['userId'] . "");
            }
            else
            {
              $mysqli->query("INSERT into player_pet (userId, level, lasersCount, shieldCount, modules) VALUES (" . $player['userId'] . ", 1, 1, 1, '[]')");
            }
  ...
  public static function GetShop()
  {
    ...
    if($pet != null)
    {
      ...
      foreach($modules as $module)
      {
        if($module->id == 11)
        {
          $comboRepModLevel = $module->level;
        }
      }
      
    }
    return [
      'categories' => ['Laser' ,'drones', 'REX', 'extras'],
      'items' => [
        [
          'id' => 0,
          'category' => 'drones',
        ...
        [
          ...
          'priceType' => 'uridium',
          'amount' => true,
          'image' => 'do_img/global/items/resource/booty-key_100x100.png',
          'active' => true
        ],
        [
          'id' => 4,
          ...
          'priceType' => 'uridium',
          'amount' => true,
          'image' => 'do_img/global/items/drone/designs/havoc_63x63.png',
          'active' => true
        ],
        [
          'id' => 5,
          ...
          'priceType' => 'uridium',
          'amount' => true,
          'image' => 'do_img/global/items/drone/designs/hercules_63x63.png',
          'active' => true
        ],
        [
          'id' => 6,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_dmg-b01_100x100.png',
          'active' => true
        ],
        [
          'id' => 7,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_dmg-b02_100x100.png',
          'active' => true
        ],
        [
          'id' => 8,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_hon-b01_100x100.png',
          'active' => true
        ],
        [
          'id' => 9,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_hon-b02_100x100.png',
          'active' => true
        ],
        [
          'id' => 10,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_xp-b01_100x100.png',
          'active' => true
        ],
        [
          'id' => 11,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_xp-b02_100x100.png',
          'active' => true
        ],
        [
          'id' => 12,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_shd-b01_100x100.png',
          'active' => true
        ],
        [
          'id' => 13,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_shd-b02_100x100.png',
          'active' => true
        ],
        [
          'id' => 14,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_hp-b02_100x100.png',
          'active' => true
        ],
        [
          'id' => 15,
        ...
        [
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_cd-b01_100x100.png',
          'active' => false
        ],
        [
          'id' => 17,
          ...
          'priceType' => 'uridium',
          'amount' => false,
          'image' => 'do_img\global\items\booster\booster_cd-b02_100x100.png',
          'active' => false
        ],
        $pet != null ? $pet['level'] < 15 ? (
        [
          'id' => 18,
          'category' => 'REX',
          'name' => 'REX_LEVEL_' . ($pet['level'] + 1),
          'price' => 500000 * ($pet['level'] + 1),
          'priceType' => 'credit',
          'amount' => false,
          'image' => 'do_img\global\items\pet\pet10-' . ($pet['level'] + 1) . '_top.png',
          'active' => true
        ] ): null :(
        [
          'id' => 18,
          ...
          'priceType' => 'credit',
          'amount' => false,
          'image' => 'do_img\global\items\pet\pet10-0_top.png',
          'active' => true
        ]),
        $modules != null ? (
        [
          'id' => 19,
          'category' => 'REX',
          'name' => 'CDR_'.$comboRepModLevel + 1,
          'price' => 1000000000 * ($comboRepModLevel + 1),
          'priceType' => 'credit',
          'amount' => false,
          'image' => 'do_img\global\items\pet_protocol_csr-02_100x100.png',
          'active' => true
        ]): [
          'id' => 19,
          'category' => 'REX',
        ...
        [
          ...
          'priceType' => 'uridium',
          'amount' => true,
          'image' => 'do_img/global/items/resource/ec_100x100.png',
          'active' => true
        ],
        [
          'id' => 21,
        ...
        [
          ...
          'priceType' => 'uridium',
          'amount' => true,
          'image' => 'do_img/global/items/equipment/generator/speed/g3n-7900_100x100.png',
          'active' => false
        ],
      ]
    ];
